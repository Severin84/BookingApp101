...
export default function CurrentlyScrapingTable({jobs}) {
  ...
  const renderCell = React.useCallback((jobs, columnKey) => {
    const cellValue = jobs[columnKey];
    //const cellValue=0;
    switch (columnKey) {
      case "url":
        return (
          <Link href={cellValue} target="_blank" className="text-primary-500">{cellValue}</Link>
        )
      case "jobType":
      ...
      case "status":
        return <Chip className="capitalize" color={statusColorMap[jobs.status]} size="sm" variant="flat">{cellValue}</Chip>  
      default:
        return cellValue;
    }
  }, []);
